...
function start() {
    for (let i = 0; i < input.length; i++) {
        ...
        for (let t = 7; t < input[i].length; t++) {
            if (input[i].charAt(t) == ",") {
                if (input[i].charAt(t-2) == " ") {
                    x1 = input[i].charAt(t-1);
                } else if (input[i].charAt(t-3) == " ") {
                    x1 = input[i].substring(t-2, t);
                } else {
                    x1 = input[i].substring(t-3, t);
                }
                if (input[i].charAt(t+2) == " ") {
                    y1 = input[i].charAt(t+1);
                } else if (input[i].charAt(t+3) == " ") {
                    y1 = input[i].substring(t+1, t+3);
                } else {
                    y1 = input[i].substring(t+1, t+4);
                }
                for (let r = t + 10; r < input[i].length; r++) {
                    if (input[i].charAt(r) == ",") {
                        if (input[i].charAt(r-2) == " ") {
                            x2 = input[i].charAt(r-1);
                        } else if (input[i].charAt(r-3) == " ") {
                            x2 = input[i].substring(r-2, r);
                        } else {
                            x2 = input[i].substring(r-3, r);
                        }
                        if (input[i].charAt(r+2) == " ") {
                            y2 = input[i].charAt(r+1);
                        } else if (input[i].charAt(r+3) == " ") {
                            y2 = input[i].substring(r+1, r+3);
                        } else {
                            y2 = input[i].substring(r+1, r+4);
                        }
                    }
                }
                break;
        ...
        for (let y = y1; y <= y2; y++) {
            for (let x = x1; x <= x2; x++) {
                if (input[i].substring(0, 6) == "toggle") {
                    lights[y][x] = !lights[y][x];
                } else if (input[i].substring(5, 7) == "on") {
                    lights[y][x] = true;
                } else if (input[i].substring(5, 8) == "off") {
                    lights[y][x] = false;
                }
            }
        }
...
function countLightsOn() {
    for (let h = 0; h < lights.length; h++) {
        for (let g = 0; g < lights[h].length; g++) {
            if (lights[h][g]) {
                lightsOn++;
            }
        }
